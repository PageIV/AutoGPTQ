...
class SupervisedDataset(Dataset): 
    ...
    def __getitem__(self, i) -> Dict[str, torch.LongTensor]:
        return dict(
...
def preprocess(sources, tokenizer): 
    ...
    for i, source in enumerate(sources): 
        ...
        for j, sentence in enumerate(source): 
            ...
            assert role == conv.roles[j % 2], f"数据对话顺序不对, 序号是: {i}"
            conv.append_message(role, sentence["value"])
        conversations.append(conv.get_prompt())
    
    # Tokenize conversations
    tokenized_convs = tokenizer(
        conversations, 
        return_tensors="pt",
        padding="max_length",
        max_length=1024,
    ...
    input_ids = tokenized_convs['input_ids']
    attention_mask = tokenized_convs['attention_mask']
    return dict(
        input_ids=input_ids,
...
def load_vicuna_data(data_path, tokenizer, n_samples):
    train_json = json.load(open(data_path, "r", encoding="utf-8"))
    train_json = random.sample(train_json, k=min(n_samples, len(train_json)))
    train_dataset = SupervisedDataset(train_json, tokenizer)
...
def main():
    ...
    examples = load_vicuna_data(data_path, tokenizer, args.num_samples)
    examples_for_quant = [
